...
Ghost::Ghost(int clr) : GameObject(GameObject::Ghost, QPixmap()) {
    ...
    ghost_death_music = new QMediaPlayer();
    ghost_death_music->setMedia(QUrl("qrc:/game_objects/sounds/Eatghost.mp3"));
    ghost_death_music->setVolume(100);
}
...
{
    setY(static_cast<int>(game->gate->y()) + 1);
    setX(static_cast<int>(game->gate->x()));
    status = Normal;
